...
- **多対多の管理関係**：1 つの Bot が複数グループを管理し、複数 Bot が同一グループを共同で
  監視・管理できます。
- **グループ単位のタスク分担**：各 Bot→グループ接続ごとに参加申請、記録、分析、対応、公告同期を設定できます。
...
| ノード | 用途 |
| --- | --- |
| QQ Bot | OneBot / NapCat アカウント、QR ログイン、接続設定 |
| Feishu Bot | CLI セッション、アーカイブ、検索 |
| QQ グループ | メッセージ、公告、審査、分析、管理関係、Bot ごとのタスク分担 |
| Feishu グループ | コラボレーション、通知、ナレッジ連携先 |
| ナレッジベース | ポリシー、運用手順、FAQ、外部知識 |

接続関係は `manages`、`observes`、`archives_to`、`searches`、`syncs` です。グラフは QQ の
グループ帰属とタスク分担に関する唯一の設定元です。`manages` / `observes` エッジ上の `tasks`
は「この Bot がこのグループで何を行うか」を表し、同じ Bot でもグループごとにスイッチ、周期、
しきい値、公告の保存先を変更できます。詳細は [オーケストレーションガイド](docs/ORCHESTRATION.md)
を参照してください。

QQ / Feishu Bot の作成、削除、接続、ログインはリソース編成画面だけで行います。右側の
インスペクターは常に 1 ノードだけを表示し、完全な設定は同じ画面のモーダルまたはノードの
コンテキストメニューから開きます。システム設定はモデル、共通ポリシー、保存、実行環境、
セキュリティに限定されます。既存 Bot の内部 ID は Webhook、Secret パス、ログイン状態を守る
ため変更できません。その Bot に接続されていないグループのイベントは Runtime キューに入る
前に Webhook 境界で破棄されます。
