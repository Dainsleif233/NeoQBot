...
飞书 CLI 面向 AI Agent，发布节奏和命令结构可能变化。MUA-Bot 将可执行文件与参数模板放在
配置中，核心服务只依赖下面两个稳定动作。
...
`--json -`）。MUA-Bot 会把完整 `{payload_json}` 写入子进程 stdin。
...
数组字段也可命名为 `data` 或 `results`。如果 CLI 只能输出普通文本，MUA-Bot 会将整段文本
作为一个结果回复，但建议使用 JSON 以保证稳定。

## 登录态

在宿主机或镜像内按官方页面完成真人账号登录，再把 CLI 登录态目录挂载到容器的
`/home/mua` 对应路径。不要把 cookie、refresh token、个人配置目录打进公开镜像。登录态过期
