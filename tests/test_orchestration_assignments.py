...
from neoqbot.models import GroupMessage, JoinDecision, JoinRequest, ModerationResult
from neoqbot.napcat import initialize_napcat, napcat_event_client_diagnostics
from neoqbot.runtime import Runtime
from neoqbot.services import AUTOMATED_REVIEWER, JoinApprovalService, ModerationService
...
class JoinReviewerRecordingTests(unittest.TestCase):
    def test_join_request_reviewer_is_persisted_and_queryable(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            database = Database(Path(directory) / "join.db")
            database.initialize()
            request = JoinRequest(
                bot_id="worker",
                event_id="evt-1",
            ...
            self.assertTrue(database.save_join_request(request))
            decision = JoinDecision(decision="approve", confidence=0.9, reason="ok")
            database.update_join_decision(request, decision, "approved", reviewer="system")

            row = database.get_join_request("flag-1", group_id="100")
            ...
            # An empty reviewer clears the attribution (manual review pending).
            database.update_join_decision(request, decision, "manual_review", reviewer="")
            pending = database.get_join_request("flag-1", group_id="100")
            assert pending is not None
            self.assertIsNone(pending["reviewer"])

            self.assertIsNone(database.get_join_request("missing-flag"))


class JoinApprovalReviewerServiceTests(unittest.IsolatedAsyncioTestCase):
    @staticmethod
    def _service(database: Database, settings: Settings) -> JoinApprovalService:
        return JoinApprovalService(
            settings,
            database,
            AsyncMock(),
            AsyncMock(),
    ...
    async def test_auto_approve_attributes_the_system_reviewer(self) -> None:
        settings = assignment_settings()
        edge = next(e for e in settings.orchestration.edges if e.id == "worker-analyze")
        edge.tasks.join_management.execute_management = True
        edge.tasks.join_management.auto_approve = True
        edge.tasks.join_management.minimum_confidence = 0.5
        settings.app.dry_run = False
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            database = Database(Path(directory) / "join.db")
            database.initialize()
            service = self._service(database, settings)
            service.engine.review_join = AsyncMock(
                return_value=JoinDecision(decision="approve", confidence=1.0, reason="ok")
            )
            request = JoinRequest(
                bot_id="worker",
                event_id="evt-1",
            ...
            status = await service.handle(request)

            self.assertEqual(status, "approved")
            service.qq.approve_join.assert_awaited_once()
            row = database.get_join_request("flag-1", group_id="200")
            assert row is not None
            self.assertEqual(row["reviewer"], AUTOMATED_REVIEWER)
            self.assertEqual(row["action_status"], "approved")

    async def test_manual_decision_records_the_operator_reviewer(self) -> None:
        settings = assignment_settings()
        ...
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            ...
            self.assertTrue(database.save_join_request(request))
            status = await service.apply_manual_decision(request, True, "looks fine", "alice")

            self.assertEqual(status, "dry_run_manual_approve")
            service.qq.approve_join.assert_awaited_once()
            row = database.get_join_request("flag-1", group_id="200")
            assert row is not None
    ...
    async def test_manual_decision_is_idempotent_for_decided_requests(self) -> None:
        settings = assignment_settings()
        settings.app.dry_run = True
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            database = Database(Path(directory) / "join.db")
            ...
            self.assertTrue(database.save_join_request(request))
            first = await service.apply_manual_decision(request, True, "looks fine", "alice")
            self.assertEqual(first, "dry_run_manual_approve")

            # A second attempt on an already-decided request must not re-invoke QQ or
            # overwrite the original reviewer.
            second = await service.apply_manual_decision(request, False, "changed mind", "bob")
            self.assertEqual(second, "dry_run_manual_approve")
            service.qq.approve_join.assert_awaited_once()
            row = database.get_join_request("flag-1", group_id="200")
            assert row is not None
            self.assertEqual(row["reviewer"], "alice")
...
class JoinReviewerEndpointTests(unittest.TestCase):
    def _build_app(self, directory: Path) -> object:
        config = assignment_settings().model_dump(mode="json")
        config["app"]["database_path"] = str(Path(directory) / "join.db")
        config["app"]["message_archive_path"] = str(Path(directory) / "messages")
        config["gui"].update({"enabled": True, "bootstrap_password": "temporary-test-password"})
        settings = Settings.model_validate(config)
        config_path = Path(directory) / "config.yaml"
    ...
    def test_gui_join_review_endpoint_records_the_logged_in_reviewer(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            app = self._build_app(Path(directory))
            ...
            with TestClient(app) as client:
                client.cookies.set("neoqbot_session", "test-session")
                response = client.post(
                    "/api/gui/orchestration/group/join/review",
                    headers={"X-CSRF-Token": "test-csrf"},
                    params={
                        "group_id": "100",
                        "flag": "flag-1",
            ...
            self.assertEqual(row["reviewer"], "alice")
            self.assertEqual(row["decision"], "approve")
            self.assertEqual(row["action_status"], "dry_run_manual_approve")

    def test_gui_join_review_endpoint_rejects_an_unknown_request(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path.cwd()) as directory:
            app = self._build_app(Path(directory))
            ...
            with TestClient(app) as client:
                ...
                response = client.post(
                    "/api/gui/orchestration/group/join/review",
                    headers={"X-CSRF-Token": "test-csrf"},
                    params={"group_id": "100", "flag": "missing", "approve": "true"},
                )
