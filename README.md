...
- **群级事务分工**：在每条 Bot→群连接上分别启用入群管理、消息记录、定时分析和公告同步。
- **清晰的采集语义**：消息“记录”和“定时分析”互相独立；同时开启时分析直接复用已记录窗口，
  不重复采集。公告只保留一个开关，启用即全量归档已有公告并持续同步。
- **群组工作台**：从 QQ 群节点右键打开类聊天窗口，集中查看群消息、公告版本、入群请求、分析记录和当前管理关系，并按时间范围清理或导出 JSON。
...
QQ/飞书 Bot 的新增、删除、账号连接和登录也只在资源编排完成。选择 QQ 群节点可在右侧查看
入群申请、群聊消息、分析报告和公告版本的数据看板，以及节点和连接的快捷开关；复杂策略、周期、
阈值和目标则在“详细配置”或右键节点打开的中心弹窗中维护。系统设置只包含模型、共享策略、
