...
群消息到达 Webhook 后只进行一次标准化采集。开启“记录”时写入 SQLite 和按日 JSONL；只开启
“定时分析”时写入 SQLite 窗口缓冲；同时开启时仍只执行一次 SQLite 插入，JSONL 也只追加一次。
记录包含 QQ 号、群名片/昵称、发送时间和文本，分析服务直接从 SQLite 读取窗口，不再次调用平台
获取历史消息。
...
1. 群连接启用公告同步后立即调用 OneBot 公告扩展动作抓取全部已有公告，之后按配置周期持续抓取；
2. 使用群号、规范化后的标题/内容哈希与 UTC 发布时间分钟槽识别同一发布版本；没有发布时间时
   保守回退到公告 ID，避免把不同时期的相同内容错误合并；
3. 多个 Bot 在同一时间看到相同内容时，只更新 `last_seen_at`、来源 Bot 和来源公告 ID 列表，
   即使平台返回不同公告 ID 也不新增重复行；
4. 新内容成为当前版本，旧内容保留为历史版本；一次完整抓取中消失的公告标记为已在群聊删除；
5. 原子领取 pending/failed 版本并调用飞书 CLI，避免并发 Bot 重复导出；
6. 成功标记 synced，失败记录错误和尝试次数，超时的领取状态会在下一轮恢复重试。
...
- 群详情通过受 GUI 会话保护的接口聚合 SQLite 中的消息、公告、分析和申请记录；分页读取为只读，
  显式清理需要 CSRF 校验并同步处理消息 JSONL，JSON 导出和清理动作都会写入审计日志。
